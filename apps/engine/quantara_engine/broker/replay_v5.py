"""Replay V5 — dual-layer P&L reconciliation, mark completion timing, session truth."""
...
from quantara_engine.broker.pnl import quote_pnl_to_usd, realized_pnl_usd
...
class ReplayV5State:
    ...
    mark_timing_method: str = "bar_close_timestamp(5m)"
    freshness_method: str = "data_fresh_for_instrument(5m)"
...
def _track_peaks(state: ReplayV5State) -> None:
...
def _attributed_for_strategy(state: ReplayV5State, symbol: str, strategy_position_id: str) -> Decimal:
    return sum(
        (
            lot.remaining_qty
...
def _attributed_unrealized(state: ReplayV5State) -> Decimal:
    total = Decimal("0")
    for symbol, queue in state.lots.items():
        if not queue:
            continue
        ...
        pnl_quote_total = Decimal("0")
        for lot in queue:
            if lot.remaining_qty <= 0:
                continue
            if lot.direction == "long":
                pnl_quote_total += (mark - lot.entry_price) * lot.remaining_qty
            else:
                pnl_quote_total += (lot.entry_price - mark) * lot.remaining_qty
        if pnl_quote_total != 0:
            total += quote_pnl_to_usd(pnl_quote_total, spec, state.fx_rates)
    return total
...
def _apply_cash_on_fill(
    ...
    *,
    ...
    if spec.asset_class == "crypto" and rules.initial_margin_pct >= Decimal("100"):
        ...
        delta = gross_pnl - fees
        state.cash += delta
        state.cash_from_fills += delta


def _apply_mark(state: ReplayV5State, symbol: str, mark: Decimal) -> None:
...
def _execute_fill(state: ReplayV5State, event: ReplayV5Event, *, is_close: bool) -> None:
...
def _event_sort_key(ev: ReplayV5Event) -> tuple:
    """Marks sort after trade events at the same instant (bar open vs bar close)."""
    ts = ev.timestamp or datetime.min
    kind_order = 1 if ev.kind == "mark" else 0
    return (ts, kind_order, ev.symbol)


def replay_v5(events: list[ReplayV5Event], *, state: ReplayV5State | None = None) -> dict:
    state = state or ReplayV5State(profile=QUANTARA_STANDARD_PAPER)
    accepted = rejected = 0
    ...
    for event in sorted(events, key=_event_sort_key):
        ...
        if event.market_open is None:
            rejected += 1
            state.unknown_session_count += 1
            state.rejection_counts["unknown_market_session"] = (
                state.rejection_counts.get("unknown_market_session", 0) + 1
            )
            if event.kind == "entry":
        ...
        if event.kind == "exit":
            ...
            if event.strategy_position_id:
                cap_qty = min(
                    event.quantity,
                    _attributed_for_strategy(state, event.symbol, event.strategy_position_id),
                )
            if cap_qty <= 0:
            ...
            if cap_qty < event.quantity:
                event = ReplayV5Event(
                    ...
                    freshness_detail=event.freshness_detail,
        ...
        if not decision.accepted:
            ...
            if reason == "stale_market_data":
                state.stale_rejection_count += 1
            elif reason == "market_closed":
                state.market_closed_rejection_count += 1
            if event.kind == "entry":
                state.broker_rejected_entries += 1
            continue
    ...
    per_asset_realized_diff: dict[str, float] = {}
    ...
    physical_unrealized = final.unrealized_pnl
    attributed_unrealized = _attributed_unrealized(state)
    physical_total_pnl = state.gross_realized + physical_unrealized
    attribution_total_pnl = state.attributed_realized + attributed_unrealized
    total_pnl_reconciliation = (physical_total_pnl - attribution_total_pnl).quantize(Decimal("0.01"))

    realized_basis_difference = (state.physical_fill_realized - state.attributed_realized).quantize(
        Decimal("0.01")
    )
    unrealized_basis_difference = (attributed_unrealized - physical_unrealized).quantize(Decimal("0.01"))
    basis_carry_check = (realized_basis_difference - unrealized_basis_difference).quantize(Decimal("0.01"))

    penny = Decimal("0.01")
    rounding_tolerance = Decimal("0.05")
    expected_netting_basis_carry = (
        abs(realized_basis_difference) >= penny
        and abs(total_pnl_reconciliation) <= rounding_tolerance
        and abs(basis_carry_check) <= rounding_tolerance
    )
    ...
    expected_cash_from_fills = state.starting_cash + state.cash_from_fills
    cash_reconciliation = (state.cash - expected_cash_from_fills).quantize(Decimal("0.01"))

    independent_im = Decimal("0")
    independent_mm = Decimal("0")
    for symbol, (qty, _avg, mark) in state.positions.items():
        spec = get_instrument_spec(symbol)
        rules = state.profile.rules_for(spec.asset_class)
    ...
    margin_reconciliation = (independent_im - final.initial_margin_used).quantize(Decimal("0.01"))
    ...
    return {
        ...
        "unknown_session_count": state.unknown_session_count,
        "stale_rejection_count": state.stale_rejection_count,
        "market_closed_rejection_count": state.market_closed_rejection_count,
        "mark_timing_method": state.mark_timing_method,
        "freshness_method": state.freshness_method,
        ...
        "physical_unrealized_pnl": float(physical_unrealized),
        "attributed_realized_pnl": float(state.attributed_realized),
        "attributed_unrealized_pnl": float(attributed_unrealized),
        "physical_total_pnl": float(physical_total_pnl),
        "attribution_total_pnl": float(attribution_total_pnl),
        "realized_basis_difference": float(realized_basis_difference),
        "unrealized_basis_difference": float(unrealized_basis_difference),
        "total_pnl_reconciliation": float(total_pnl_reconciliation),
        "basis_carry_check": float(basis_carry_check),
        "expected_netting_basis_carry": expected_netting_basis_carry,
        "financial_reconciliation": float(financial_reconciliation),
        "cash_reconciliation": float(cash_reconciliation),
        ...
        "per_asset_realized_basis_difference": per_asset_realized_diff,
...
def _data_fresh_for_row(store: TradingStore, symbol: str, asset_class: str, at: datetime) -> tuple[bool, str]:
    ...
    try:
        ...
        fresh, detail = data_fresh_for_instrument(store, instrument, _MARK_TIMEFRAME, at)
...
def _load_mark_events(
    ...
    rows = store.session.execute(
        ...
        {"d0": day_start, "d1": day_end, "tf": _MARK_TIMEFRAME, "symbols": sorted(symbols)},
    ).mappings().all()
    return [
        ReplayV5Event(
            kind="mark",
            ...
            timestamp=bar_close_timestamp(r["timestamp"], _MARK_TIMEFRAME),
        )
        for r in rows
    ]
...
def _build_trade_events(
    ...
    *,
...
) -> tuple[list, list, list[ReplayV5Event]]:
    date_filter_entries = ""
    date_filter_exits = ""
    params: dict = {"exp_ids": list(_COMPETITION_EXPERIMENTS)}
    if day_start is not None and day_end is not None:
        date_filter_entries = "AND p.opened_at >= :d0 AND p.opened_at < :d1"
        date_filter_exits = "AND t.closed_at >= :d0 AND t.closed_at < :d1"
        params["d0"] = day_start
        params["d1"] = day_end
    ...
    for e in entries:
        ...
        if audit_fast:
            fresh, fresh_detail = True, "audit_historical_skip_live_freshness_gate"
        else:
            fresh, fresh_detail = _data_fresh_for_row(store, sym, ac, ts)
        events.append(
            ReplayV5Event(
                kind="entry",
                symbol=sym,
...
def replay_historical_day(
    ...
    *,
    ...
    state = ReplayV5State(
        ...
        starting_cash=start,
        mark_timing_method=f"bar_close_timestamp({_MARK_TIMEFRAME})",
        freshness_method=f"data_fresh_for_instrument({_MARK_TIMEFRAME})",
    )

    entries, exits, events = _build_trade_events(
        store, day_start=day_start, day_end=day_end, audit_fast=True
    )
...
def write_replay_v5_audit_markdown(
    store: TradingStore, path: str, day_start: datetime, day_end: datetime
) -> dict:
    result = replay_historical_day(store, day_start, day_end)
    lines = [
        f"# Broker Replay V5 — {day_start.date()}",
        "",
        "## Summary",
    ...
    lines.extend([
        "",
        "## Methodology",
        f"- Mark timing: {result.get('mark_timing_method', 'unknown')}",
        f"- Freshness: {result.get('freshness_method', 'unknown')}",
        f"- Unknown market session rejections: {result.get('unknown_session_count', 0)}",
        f"- Stale data rejections: {result.get('stale_rejection_count', 0)}",
        f"- Market closed rejections: {result.get('market_closed_rejection_count', 0)}",
        "",
        "## Ending state",
        f"- Ending cash: ${result.get('ending_cash', 0):,.2f}",
        f"- Ending balance: ${result.get('ending_balance', 0):,.2f}",
        f"- Ending equity: ${result.get('ending_equity', 0):,.2f}",
        "",
        "## Physical broker accounting",
        f"- Physical gross realized: ${result.get('gross_realized_pnl', 0):,.2f}",
        f"- Fees: ${result.get('fees_paid', 0):,.2f}",
        f"- Physical net realized: ${result.get('net_realized_pnl', 0):,.2f}",
        f"- Physical unrealized: ${result.get('physical_unrealized_pnl', 0):,.2f}",
        f"- Physical total P&L: ${result.get('physical_total_pnl', 0):,.2f}",
        "",
        "## Strategy attribution accounting",
        f"- Attributed strategy realized: ${result.get('attributed_realized_pnl', 0):,.2f}",
        f"- Attributed strategy unrealized: ${result.get('attributed_unrealized_pnl', 0):,.2f}",
        f"- Attributed total P&L: ${result.get('attribution_total_pnl', 0):,.2f}",
        "",
        "## Dual-layer basis reconciliation",
        f"- REALIZED BASIS DIFFERENCE (physical − attributed): ${result.get('realized_basis_difference', 0):,.2f}",
        f"- UNREALIZED BASIS DIFFERENCE (attributed − physical): ${result.get('unrealized_basis_difference', 0):,.2f}",
        f"- Basis carry check (realized − unrealized): ${result.get('basis_carry_check', 0):,.2f}",
        f"- TOTAL P&L RECONCILIATION (physical − attributed): ${result.get('total_pnl_reconciliation', 0):,.2f}",
    ])
    if result.get("expected_netting_basis_carry"):
        lines.append(
            "- **EXPECTED NETTING BASIS CARRY** — realized difference explained by remaining-lot basis; not a failure."
        )
    elif result.get("realized_basis_difference", 0) != 0:
        lines.append("- Realized basis difference does NOT reconcile via unrealized carry — investigate.")
    lines.extend([
        "",
        f"- Financial reconciliation (balance): {result.get('financial_reconciliation', 0):.2f}",
        f"- Cash reconciliation: {result.get('cash_reconciliation', 0):.2f}",
        f"- Initial margin reconciliation: {result.get('margin_reconciliation', 0):.2f}",
        f"- Maintenance margin reconciliation: {result.get('maintenance_reconciliation', 0):.2f}",
        "",
        "## FX methodology",
